...
# ------------------------------------------------------------ project config
# A project can override ticket-system defaults via scripts/ticket.config.sh
# (generated by the ticket-system-bootstrap skill) or via TKT_* env vars.
# With neither, the form-gen defaults below apply — behavior is unchanged.
# NOTE: ${VAR-default} (not :-) so an explicitly-empty override (e.g.
# TKT_MIGRATE_CMD="") stays empty instead of falling back to the default.
...
DEFAULT_ASSIGNEE="${TKT_DEFAULT_ASSIGNEE-jarvis}"
PORT_BASE="${TKT_PORT_BASE-3100}"
TEST_CMD="${TKT_TEST_CMD-npx vitest run}"
...
cmd_start() {
  ...
  if [[ -n "$TEST_CMD" ]]; then
    if [[ -n "$DB_URL_BASE" ]]; then
      echo "test command: DATABASE_URL=\"$DB_URL_BASE/$db\" $TEST_CMD"
    else
      echo "test command: $TEST_CMD"
    fi
  else
    echo "test command: not configured (TKT_TEST_CMD empty)"
  fi
  # Leading zeros parse as octal in bash arithmetic ("018" → error) — force base 10.
  echo "server port: $((PORT_BASE + 10#${id#TKT-}))"
}
...
cmd_db_create() {
  ...
  db="${TEST_DB_PREFIX}${id#TKT-}"
  if [[ -z "$DB_URL_BASE" || -z "$MIGRATE_CMD" ]]; then
    echo "db provisioning not configured (TKT_DB_URL_BASE / TKT_MIGRATE_CMD empty) — skipping per-ticket DB"
    return 0
  fi
  ...
  DATABASE_URL="$DB_URL_BASE/$db" $MIGRATE_CMD >/dev/null 2>&1
  echo "migrated db $db"
}
